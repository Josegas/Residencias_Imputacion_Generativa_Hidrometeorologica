...
### Modo datos externos (`--output-dir`)

Al procesar archivos `.txt` de una fuente distinta a CONAGUA, es posible especificar un directorio de salida alternativo para no contaminar los datos originales de Sinaloa:

```bash
python organize_raw_by_station_year_variable_parquet.py \
    --output-dir data/interim/organized_external/estado=sin
```

| Argumento | Descripción |
|-----------|-------------|
| `--output-dir` | Directorio de salida alternativo (relativo a la raíz del proyecto o ruta absoluta). Si se omite, se usa el valor configurado en `config.yaml` (`data/interim/organized/estado=sin`). |
