...
ESTADO = _CFG["estado"]
BASE_URL = _CFG["url"]["base"]
USER_AGENT = _CFG["url"]["user_agent"]
STATION_RANGE_START = _CFG["estaciones"]["rango_inicio"]
STATION_RANGE_END = _CFG["estaciones"]["rango_fin"]
REQUEST_TIMEOUT = _CFG["conexion"]["timeout_seg"]
RETRIES_PER_PASS = _CFG["conexion"]["reintentos_por_pase"]
SLEEP_BETWEEN_CLAVES = _CFG["conexion"]["pausa_entre_claves"]
MAX_WORKERS          = _CFG["conexion"]["max_workers"]
MAX_HTML_PREVIEW     = _CFG["conexion"]["max_html_preview"]
CONTENT_SAMPLE_BYTES = _CFG["conexion_avanzada"]["content_sample_bytes"]
BACKOFF_BASE         = _CFG["conexion_avanzada"]["backoff_base"]


def _find_project_root(start: Path) -> Path:
    for directory in [start, *start.parents]:
        if (directory / ".project-root").exists():
            return directory
    raise RuntimeError(
        f"No se encontró '.project-root' buscando desde {start}. "
        "Asegúrate de que el archivo marcador existe en la raíz del proyecto."
    )
...
_r = _CFG["rutas"]
DATA_ROOT = PROJECT_ROOT / _r["data_root"] / f"estado={ESTADO}"
RAW_ROOT  = DATA_ROOT / _r["fuente"] / _r["producto"]
LOG_DIR   = DATA_ROOT / _r["logs"]
META_DIR  = DATA_ROOT / _r["meta"]

_af = _r["archivos"]
MANIFEST_PATH      = LOG_DIR / _af["manifest"]
RUNLOG_PATH        = LOG_DIR / _af["runlog"]
SUMMARY_PATH       = LOG_DIR / _af["summary"]
STATIONS_META_PATH = META_DIR / _af["stations_meta"].format(estado=ESTADO)
...
def _validate_content(content: bytes):
    ...
    sample = content[:CONTENT_SAMPLE_BYTES].decode("latin-1", errors="ignore").lower()
...
def process_range(claves: list[int], pass_no: int) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(
            _download_and_log, clave, pass_no): clave for clave in claves}
...
def summarize(final_results: list[dict]) -> list[tuple]:
    ...
    fail = sum(1 for r in final_results if r["status"] not in (
        "OK", "SKIP_EXISTS", "HTTP_404"))
